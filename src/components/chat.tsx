    ...
    <div className={classNames("col-12 cursor-pointer", className)} {...props}>
      <div
        className={classNames(
          ...
          "transition-base",
          {
            "bg-l25-info-hover": !theme,
            "bg-lo50-info-hover": theme,
            "bg-l10-info": !theme && isActive,
            "bg-lo25-info": theme && isActive,
          },
        )}
      >
        <ChatAvatar
...
export const ChatGroup: FC<IChatGroupProps> = ({
  ...
  const AVATAR = (
    <ChatAvatar src={user.src} isOnline={user.isOnline} color={user.color} />
  );
  return (
    // eslint-disable-next-line react/jsx-props-no-spreading
    <div
      className={classNames("chat-group", { "chat-group-reply": isReply })}
      {...props}
    >
      {!isReply && AVATAR}
